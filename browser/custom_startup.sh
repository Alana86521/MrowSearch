...
pulse_home=${MROW_PULSE_HOME:-/var/run/pulse}
start_pulse() {
    mkdir -p "$pulse_home"
    chmod 0700 "$pulse_home"
    if ! HOME="$pulse_home" pulseaudio --check >/dev/null 2>&1
    then
        HOME="$pulse_home" pulseaudio --start
    fi
}
start_pulse
